...
> **Charter status**: active · **Version**: 0.2 · **Last updated**: 2026-06-15
...
- CLI — `flexspec init`, `flexspec new`, `flexspec config` (read table/JSON; `config set` to update), `flexspec list` (`--json`; task counts from spec `task_count` frontmatter, with computed fallback from simple task rows/bullets or `tasks/` files), `flexspec validate` (warns on `task_count` drift), `flexspec update` (upgrade CLI, reinstall skills, run migrations including `task-count` backfill; `--dry-run`, `--check`), `flexspec ui` (local management dashboard), `flexspec status set` (update spec/task status in frontmatter), `flexspec glossary list`/`query`/`add` (project glossary management), `flexspec brainstorm new <name> [--force]` (scaffold `.flexspec/brainstorms/<slug>.md` from a project-local template for pre-spec exploration; no status lifecycle, not returned by `flexspec list` or shown in the UI board). Human-readable output for `list`, `config`, `validate`, `update`, and `glossary` uses aligned column tables with headers; `--json` on list/config/glossary for scripts and agents.
...
- Agent skills — `/flexspec` (spec lifecycle with task-type taxonomy, per-type section relaxation, automatic charter updates, glossary awareness, brainstorm-doc auto-detect/ingest, and Phase 3 slop delegation), `/flexspec-charter` (application charter and glossary discovery handoff), `/flexspec-migrate` (convert specs from other SDD tools into FlexSpec format), `/flexspec-glossary-discovery` (scan project language, ask for unclear term meanings, record confirmed definitions via `flexspec glossary scan`), `/flexspec-slop-cleanup` (AI slop reviewer invoked by `/flexspec` Phase 3 and usable standalone), and `/flexspec-brainstorm` (in-depth pre-spec interview covering problem framing, edge cases, security, performance, alternatives, and open questions; writes a brainstorm doc under `.flexspec/brainstorms/` that `/flexspec` Phase 1 auto-detects and cites, without re-asking already-answered topics), including structured multiple-choice interviews for ambiguous requests and UI-heavy specs. The ambiguity gate clarifies goals, scope, success criteria, constraints, and unknowns before planning; the UI design gate covers visual identity, layout system, component library, motion/feedback, UX flows/content, and accessibility/input. `/flexspec` updates `.flexspec/charter.md` automatically for in-scope charter deltas instead of asking whether to update it; `/flexspec-charter` invokes glossary discovery so charter terminology and `.flexspec/glossary.yaml` stay aligned. Specs carry a `type` field (feature, bug, chore, refactor, docs, infra, spike, research) that drives which sections are required and how deep the discovery gate runs; spike and research types terminate at `proposed` where the plan is the deliverable.
...
| Spec type | `type` frontmatter field classifying a spec's kind: `feature`, `bug`, `chore`, `refactor`, `docs`, `infra`, `spike`, or `research`. Drives which sections are required and how deep the discovery gate runs. Inferred by `/flexspec` when omitted; `flexspec validate` warns on missing/invalid values. |
| Proposed | Terminal spec status for plan-only deliverables (spike, research) where the plan is the output, not a prelude to implementation. Sits alongside `complete`; no Phase 2/3 runs for specs that end here. |
| Slop review | `/flexspec-slop-cleanup` skill that detects AI slop (semantic duplication, complexity inflation, test-implementation coupling, architectural drift, useless comments). Invoked by `/flexspec` Phase 3 and usable standalone. Supersedes the user-installed `ai-slop` skill. |
| Brainstorm doc | A `.flexspec/brainstorms/<slug>.md` file capturing an in-depth pre-spec interview (problem, edge cases, security, performance, alternatives, open questions) for a feature idea; created via `flexspec brainstorm new` and authored by `/flexspec-brainstorm`. No status lifecycle; not returned by `flexspec list` or the UI board. |
| Brainstorm skill | `/flexspec-brainstorm` — agent skill that interviews the user to explore a feature idea before spec authoring, producing a brainstorm doc that `/flexspec` Phase 1 can auto-detect and ingest as read-only discovery context. |
...
| 2026-06-15 | §4 — spec templates now use exhaustive discovery, workflow graphs, stepwise implementation plans, test plans, FR/NF requirements, and task tables; simple task counts include table rows. | direct request |
| 2026-06-17 | §4/§9 — task-type taxonomy (`type` field: feature/bug/chore/refactor/docs/infra/spike/research), `proposed` terminal status for plan-only deliverables, `/flexspec-slop-cleanup` skill added to registry and delegated to by Phase 3, `flexspec glossary scan` subcommand, glossary interview scoped to Phase 1, one-shot stop-and-ask semantics, token budgets advisory-only. | direct request |
| 2026-07-01 | §4/§9 — `flexspec brainstorm new` CLI command and `/flexspec-brainstorm` skill for pre-spec exploration (problem framing, edge cases, security, performance, alternatives); `/flexspec` Phase 1 auto-detects and ingests a matching brainstorm doc as read-only discovery context. | 016-flexspec-brainstorm-skill |
