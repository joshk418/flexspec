...
This spec changes the default orchestration so the CLI update step runs first, followed by skills and then migrations. The intended outcome is that a bare `flexspec update` updates the installed CLI before doing the rest of the work, allowing the freshly installed binary/versioned behavior to participate in the same user workflow as far as the current process model allows.
...
| `internal/selfupdate/selfupdate.go` | reference | Existing CLI/skills plan and apply helpers remain unchanged |
...
- **FR-001** — Bare `flexspec update` runs selected work in this order: CLI update first, skills update second, migrations third.
...
- **T-001** — Reorder `cmd/update.go::runUpdate` so CLI actions are planned/applied before skills, and migrations run after self-update actions _(satisfies: FR-001, FR-003, FR-004, NF-002; files: `cmd/update.go`; depends_on: none; §2.2 steps: 3-16)_.
...
- **T-003** — Add/update `cmd/update_test.go` coverage for default dry-run/report order, applied runner order, `--check` migration-only behavior, and unchanged step flag selection _(satisfies: FR-001, FR-002, FR-003, FR-004, FR-005, NF-001; files: `cmd/update_test.go`; depends_on: T-001, T-002; §2.2 steps: 2-19)_.
...
| TC-002 | FR-001, NF-002 | Applied update with injected runner records `go` before `npx`, with migrations happening after self-update. | unit |
